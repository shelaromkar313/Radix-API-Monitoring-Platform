...
import { IncidentCategoryType, IncidentSeverityType, MetricSnapshot } from './types.js';
...
export class IncidentService {
  ...
  static async createIncident(input: {
    ...
    if (existing) {
      ...
      await prisma.incident.update({
        ...
        data: {
          metrics_snapshot: input.metrics as any,
          severity: input.severity as any // update severity if elevated
        }
      });
      ...
      await prisma.incidentEvent.create({
        data: {
          ...
          metadata: { metrics: input.metrics } as any,
    ...
    const incident = await prisma.incident.create({
      data: {
        ...
        status: 'OPEN',
        category: (input.category || 'UNKNOWN') as any,
        metrics_snapshot: input.metrics as any
      },
      include: {
        endpoint: true,
  ...
  static async getIncidents(params: {
    ...
    if (params.endpointId) where.endpoint_id = params.endpointId;
    if (params.severity) where.severity = params.severity;
    if (params.status) where.status = params.status;
    if (params.category) where.category = params.category;
    if (params.startDate || params.endDate) {
      where.detected_at = {};
      if (params.startDate) where.detected_at.gte = new Date(params.startDate);
      if (params.endDate) where.detected_at.lte = new Date(params.endDate);
    }
   ...
   */
  static async getIncidentStats(projectId?: string) {
    const where: any = projectId ? { project_id: projectId } : {};
   ...
   */
  static async simulateOutageDemo(projectId?: string, userId?: string) {
