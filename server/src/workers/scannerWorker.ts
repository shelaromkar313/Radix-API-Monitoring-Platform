...
const scanLiveService = async (projectId: string, liveUrl: string, io: any) => {
  ...
  if (io) io.to(projectId).emit('status_update', { status: 'scanning', message: 'Connecting to live service...' });
  ...
  if (discoveredRoutes.length > 0) {
    await prisma.endpoint.createMany({
      data: discoveredRoutes.map(route => ({
        ...
        method: route.method,
        path: route.path,
        request_schema: (route.request_schema || null) as any,
        response_schema: (route.response_schema || null) as any
      }))
    });
  }
  ...
  if (io) io.to(projectId).emit('status_update', { status: 'completed', message: 'Live service connected and endpoints ready' });
...
export const processScanJob = async (projectId: string, repositoryUrl: string) => {
  ...
  try {
    ...
    if (io) io.to(projectId).emit('status_update', { status: 'scanning', message: 'Scanning started' });
    ...
    if (io) io.to(projectId).emit('status_update', { status: 'processing', message: 'Analyzing code structure...' });
    ...
    if (io) io.to(projectId).emit('status_update', { status: 'completed', message: 'Scanning completed successfully' });
    ...
    if (io) io.to(projectId).emit('status_update', { status: 'failed', message: 'Scanning failed' });
  } finally {
    if (fs.existsSync(tempDir)) {
      fs.rmSync(tempDir, { recursive: true, force: true });
    }
